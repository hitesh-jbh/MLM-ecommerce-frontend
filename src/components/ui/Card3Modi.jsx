...
const Card3Modi = ({ product }) => {
  ...
  // 1. Fetch the actual wishlist from the server
  // This is the "Source of Truth" that prevents duplicate POSTs
  ...
  // 2. Check if this product is in the fetched wishlist
  // Using String() conversion ensures "6" matches 6
  ...
  const handleWishlistToggle = async (e) => {
    ...
    // Determine action based on real server data
    const loadingToast = toast.loading(isLiked ? "Removing..." : "Adding...");
    
    try {
      if (isLiked) {
        // Correctly calls DELETE /api/wishlist/6
        ...
        // Correctly calls POST /api/wishlist/
        await addToWishlist(token, productId);
        toast.update(loadingToast, { render: "Added!", type: "success", isLoading: false, autoClose: 2000 });
      }

      // 3. Re-validate the wishlist cache
      // This immediately updates 'isLiked' and the heart color
