...
export default function Nav() {
  ...
  // FIXED: Removed 'user' from dependencies to prevent infinite loops
  useEffect(() => {
    ...
    validateSession();
  }, [dispatch]); // Only dispatch is needed here
  ...
  // NEW: Dynamic Profile Path Logic
        ...
        <div className="relative group">
          ...
          <div className="absolute top-10 right-0 hidden group-hover:flex flex-col bg-white border border-gray-200 shadow-xl rounded-lg p-4 min-w-[220px] z-[100]">
            ...
            <div className="pt-3 border-t border-gray-100 space-y-2">
              {/* Extra Admin link if applicable */}
              {(user.role === 'super_admin') && (
                <Link to="/admin/dashboard" className="text-[10px] font-bold text-blue-600 block uppercase mb-1">Admin Panel</Link>
              )}
              <button 
                onClick={() => dispatch(logout())}
                className="w-full py-1.5 bg-black text-white text-[10px] font-bold rounded uppercase tracking-widest hover:bg-gray-800 transition"
              >
                Logout
              </button>
            </div>
