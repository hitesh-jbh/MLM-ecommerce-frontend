...
function UserMgt() {
    ...
    useEffect(() => {
        ...
        const { role, user_type } = editPayload;
        ...
        // UserType to Role Mapping (Vice Versa)
        const typeToRoleMap = {
            customer: "customer",
        ...
        // If Role changes, update User Type
        if (roleToTypeMap[role] && roleToTypeMap[role] !== user_type) {
            setEditPayload(prev => ({ ...prev, user_type: roleToTypeMap[role] }));
        } 
        // If User Type changes, update Role
        else if (typeToRoleMap[user_type] && typeToRoleMap[user_type] !== role) {
            setEditPayload(prev => ({ ...prev, role: typeToRoleMap[user_type] }));
        }
    }, [editPayload.role, editPayload.user_type, isEditModalOpen]);
    ...
    const handleUpdatePermissions = async () => {
        ...
        try {
            ...
            await updateStaff(token, staffData);
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    onChange={(e) => setEditPayload(prev => ({...prev, user_type: e.target.value}))}
                                >
                                    <option value="customer">Customer</option>
