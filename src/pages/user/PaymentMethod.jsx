  ...
  const handleAddPayment = (e) => {
    ...
    if (activeTab === 'card') {
      ...
      payload = {
        type: 'upi',
        upiId: data.upiId,
        holderName: data.holderName || 'UPI User',
      };
    }
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {payments.length === 0 ? (
          <div className="col-span-full border-2 border-dashed border-gray-100 rounded-3xl py-20 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="mt-4 text-xs font-bold uppercase tracking-widest">No saved methods found</p>
          </div>
        ) : (
          payments.map((method) => (
            <div 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleAddPayment} className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl">
            <div className="flex gap-4 mb-8 border-b border-gray-100 pb-2">
              <button 
                type="button"
                onClick={() => setActiveTab('card')}
                className={`text-[10px] font-black uppercase tracking-widest pb-2 ${activeTab === 'card' ? 'border-b-2 border-black' : 'text-gray-400'}`}
              >
                Card
              </button>
              <button 
                ...
                className={`text-[10px] font-black uppercase tracking-widest pb-2 ${activeTab === 'upi' ? 'border-b-2 border-black' : 'text-gray-400'}`}
              >
                UPI
              </button>
            </div>
            ...
            <div className="space-y-6">
              {activeTab === 'card' ? (
                <>
                  <input required name="cardNumber" maxLength="16" placeholder="CARD NUMBER  WITHOUT SPACE" className="w-full border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase tracking-widest" />
                  ...
                  <div className="flex gap-4">
                    <select required name="expMonth" className="w-full border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase">
                      <option value="">MONTH</option>
                      {Array.from({ length: 12 }, (_, i) => (
                        <option key={i} value={String(i + 1).padStart(2, '0')}>{String(i + 1).padStart(2, '0')}</option>
                      ))}
                    </select>
                    <select required name="expYear" className="w-full border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase">
                      <option value="">YEAR</option>
                      {[2024, 2025, 2026, 2027, 2028, 2029, 2030].map(year => (
                        <option key={year} value={String(year).slice(-2)}>{year}</option>
                      ))}
                    </select>
                    <input required name="cvv" maxLength="3" type="password" placeholder="CVV" className="w-1/2 border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase tracking-widest" />
                  </div>
                </>
              ) : (
                <>
                  <input required name="upiId" placeholder="UPI ID (e.g. user@okaxis)" className="w-full border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase tracking-widest" />
                  <input required name="holderName" placeholder="ACCOUNT HOLDER NAME" className="w-full border-b-2 border-gray-100 py-2 focus:border-black outline-none text-xs font-bold uppercase tracking-widest" />
                </>
              )}
            </div>
            ...
            <div className="flex gap-4 mt-12">
              ...
              <button type="submit" className="flex-1 py-4 text-[10px] font-black uppercase bg-black text-white rounded-xl shadow-xl shadow-black/10">Save Method</button>
            </div>
          </form>
        </div>
      )}
